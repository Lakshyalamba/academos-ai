...
import { getEmptyNextClassState, getNextClassFromSnapshot } from "./next-class";
...
export async function getTodayOverview({
  ...
  if (!enabled) {
    return {
      ...
      nextClass: getEmptyNextClassState(
        "Connect Newton MCP locally to load your next class.",
      ),
    };
  }
  ...
  try {
    ...
    const nextClass = getNextClassFromSnapshot(snapshot);

    let message;

    if (classesToday.length > 0) {
      message = `You have ${formatCountLabel(classesToday.length, "class", "classes")} scheduled today.`;
    } else if (nextClass.available && nextClass.subjectName) {
    ...
    return {
      ...
      overdueItemsCount: null,
      nextClass: getEmptyNextClassState(
        "Your next class is unavailable until live schedule data can be loaded.",
      ),
    };
  }
}
