...
function getUnavailableMessage(error) {
  ...
  if (/not available locally|add it to codex/i.test(message)) {
    return "Live academic sync is unavailable in this deployment, so contest history is not shown here.";
  }
...
export async function getContestPageData({ enabled = true } = {}) {
  if (!enabled) {
    return getEmptyContestPageData(
      "Live academic sync is unavailable in this deployment. You can still use the planner and fallback contest guidance.",
    );
  }
