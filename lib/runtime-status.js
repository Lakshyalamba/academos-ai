...
const DEV_SERVER_COMMAND = "npm run dev -- --hostname 127.0.0.1 --port 3000";
...
function buildMissingSteps({
  ...
  if (!llmConfigured) {
    missing.push("Add a real GEMINI_API_KEY or GOOGLE_API_KEY in .env.local.");
  }
...
export function getRuntimeStatus() {
  ...
  if (!supabaseConfigured) {
    optional.push(
      "Optional: add SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in .env.local to persist snapshots.",
    );
  }
