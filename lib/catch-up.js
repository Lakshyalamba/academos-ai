...
function getUnavailableMessage(error) {
  ...
  if (/not available locally|add it to codex/i.test(message)) {
    return "Live academic sync is unavailable in this deployment, so catch-up activity is shown in fallback mode.";
  }
...
export async function getCatchUp({
  ...
  if (!enabled) {
    return getEmptyCatchUpState(
      "Live academic sync is unavailable in this deployment, so catch-up activity is shown in fallback mode.",
    );
  }
