...
function detectIntent(query) {
    const normalizedQuery = normalizeText(query);
    const asksCourses = /\bcourses?\b|\benrolled\b|\bplacement\b|\bother courses\b|\bother course\b/.test(normalizedQuery);
    const asksAttendance = /\battendance\b|\battend\b/.test(normalizedQuery);
    const asksAssignments = /\bassignment\b|\bassignments\b|\bpending\b|\bdue\b|\bhomework\b|\bsubmission\b|\bsubmissions\b/.test(normalizedQuery);
    const asksContests = /\bcontest\b|\bcontests\b|\bcoding contest\b|\bmid sem\b/.test(normalizedQuery);
    const asksQuizzes = /\bscore\b|\bscores\b|\bmarks\b|\bmark\b|\bquiz\b|\bquizzes\b|\bassessment\b|\bassessments\b|\bmcq\b|\bresult\b|\bresults\b|\btest score\b/.test(normalizedQuery);
    const asksScorecard = /\bscorecard\b/.test(normalizedQuery);
    const asksSubjectPerformance = /\ball subjects\b|\beach subject\b|\bsubject wise\b|\bsubject wise\b|\bsubject performance\b|\btopic wise\b|\btopic-wise\b/.test(normalizedQuery);
    ...
    const asksCalendar = /\bcalendar\b|\bevents\b|\bwhat s scheduled\b|\bwhat is scheduled\b|\bscheduled on\b/.test(normalizedQuery);
    const asksTimeline = /\btimeline\b|\brecent lectures\b|\bpast lectures\b|\brecent classes\b|\bwhat lectures did i miss\b|\bmissed lectures\b|\brecent activity\b/.test(normalizedQuery);
    const asksArena = /\barena\b|\bpractice\b|\bproblems solved\b|\bquestions solved\b|\bcoding stats\b|\bsolve count\b/.test(normalizedQuery);
    const asksQotd = /\bqotd\b|question of the day|daily question|daily challenge|\bmy streak\b/.test(normalizedQuery);
    const asksExpertSessions = /\bexpert session\b|\bexpert sessions\b/.test(normalizedQuery);
...
function simplifyCourseCatalog(courseData) {
    ...
    return courses.map((course)=>({
            ...
            subjects: Array.isArray(course.subjects) ? course.subjects.map((subject)=>simplifySubject(subject)) : []
        }));
}
...
function simplifyAssessmentItems(payload) {
    ...
    return Array.isArray(payload.assessments) ? payload.assessments.map((assessment)=>({
...
function simplifyRecentLectures(payload) {
    ...
    return {
        ...
        lectures: Array.isArray(payload.lectures) ? payload.lectures.map((lecture)=>({
...
function simplifyQotdHistory(payload) {
    ...
    return {
        ...
        pastQuestions: Array.isArray(payload.past_questions) ? payload.past_questions.map((question)=>({
...
function simplifyAttendance(context, overview, subjectProgresses = []) {
    const performance = context?.subject ? subjectProgresses.find((progress)=>progress.subjectHash === context.subject.subject_hash)?.performance : overview?.performance;
...
function flattenAssessments(collections) {
    return collections.flatMap((collection)=>(collection.assessments || []).map((assessment)=>({
                ...assessment,
...
function buildSubjectPerformanceLabel(entry) {
    ...
    if (typeof performance.lectures_attended === "number" && typeof performance.total_lectures === "number" && performance.total_lectures > 0) {
        segments.push(`attendance ${performance.lectures_attended}/${performance.total_lectures} (${formatPercentage(performance.lectures_attended, performance.total_lectures)})`);
    }
    if (typeof performance.completed_assignment_questions === "number" && typeof performance.total_assignment_questions === "number" && performance.total_assignment_questions > 0) {
        segments.push(`assignments ${performance.completed_assignment_questions}/${performance.total_assignment_questions}`);
    }
    if (typeof performance.completed_assessments === "number" && typeof performance.total_assessments === "number" && performance.total_assessments > 0) {
        segments.push(`quizzes ${performance.completed_assessments}/${performance.total_assessments}`);
    }
...
function buildAssignmentsResponse(snapshot) {
    ...
    if (!snapshot.context.subject && snapshot.overview?.performance?.completed_assignment_questions !== undefined) {
...
function buildContestResponse(snapshot) {
    ...
    const scope = getScopeLabel(snapshot);
    if (contests.length === 0 && upcomingContests.length === 0 && (snapshot.context.subject || typeof performance?.total_contest_questions !== "number")) {
        return buildDataNotFoundResponse();
    }
    const tasks = [];
    const insights = [
        "Contests are separate from assignments and quizzes."
    ];
    if (!snapshot.context.subject && typeof performance?.completed_contest_questions === "number" && typeof performance?.total_contest_questions === "number" && performance.total_contest_questions > 0) {
        insights.push(`Contest performance is ${performance.completed_contest_questions}/${performance.total_contest_questions} completed questions.`);
    }
    if (upcomingContests[0]) {
        tasks.push(`Prepare for ${truncateTitle(upcomingContests[0].title, 72)}${upcomingContests[0].startsAt ? ` on ${upcomingContests[0].startsAt}` : ""}.`);
        insights.push(`The next scheduled contest is ${truncateTitle(upcomingContests[0].title, 72)}${upcomingContests[0].startsAt ? ` on ${upcomingContests[0].startsAt}` : ""}.`);
    }
    if (contests[0]) {
        tasks.push(`Review ${truncateTitle(contests[0].title, 72)} for ${contests[0].subjectName}${contests[0].dueAt ? ` by ${contests[0].dueAt}` : ""}.`);
    }
    if (tasks.length === 0) {
        tasks.push(`Open the contest section for ${scope} and review the latest contest entry.`);
    }
    return {
        summary: contests[0] || upcomingContests[0] ? `I found ${formatCountLabel(contests.length, "contest", "contests")} for ${scope}${upcomingContests[0] ? `, and the next scheduled contest is ${truncateTitle(upcomingContests[0].title, 68)}` : ""}.` : `Contest performance data is available for ${scope}.`,
        tasks: tasks.slice(0, 3),
        insights: insights.slice(0, 3)
    };
}
...
function buildPerformanceResponse(snapshot) {
    ...
    if (typeof contestDone === "number" && typeof contestTotal === "number" && contestTotal > 0) {
...
function buildQuizResponse(snapshot) {
    ...
    const tasks = [];
    const insights = [
        "Quizzes are tracked separately from assignments and contests."
    ];
    if (latestAssessment.endsAt) {
        tasks.push(`Review ${truncateTitle(latestAssessment.title, 64)}${latestAssessment.subjectName ? ` for ${latestAssessment.subjectName}` : ""} before ${latestAssessment.endsAt}.`);
    }
    if (completedAssessments[1]) {
        tasks.push(`Revisit ${truncateTitle(completedAssessments[1].title, 64)}${completedAssessments[1].subjectName ? ` in ${completedAssessments[1].subjectName}` : ""} to improve consistency.`);
    }
    if (tasks.length === 0) {
        tasks.push(`Open the latest quiz or assessment for ${scope} and review the missed questions.`);
    }
    if (totalEarnable > 0) {
        insights.push(`Across ${completedAssessments.length} quizzes, you have ${totalEarned}/${totalEarnable} points overall.`);
    }
    insights.push(`${fullScores} ${fullScores === 1 ? "quiz has" : "quizzes have"} a full score in this scope.`);
    return {
        summary: typeof latestAssessment.earnedPoints === "number" && typeof latestAssessment.earnablePoints === "number" ? `Your latest quiz result${latestAssessment.subjectName ? ` in ${latestAssessment.subjectName}` : ""} is ${latestAssessment.earnedPoints}/${latestAssessment.earnablePoints} in ${truncateTitle(latestAssessment.title, 68)}.` : `I found quiz data for ${scope}, but the latest score entry is incomplete.`,
        tasks: tasks.slice(0, 3),
        insights: insights.slice(0, 3)
    };
}
function buildCoursesResponse(snapshot) {
    ...
    const otherCourses = courses.filter((course)=>course.courseHash !== snapshot.context.course?.courseHash);
    const placementCourses = courses.filter((course)=>/placement/i.test(`${course.courseTitle || ""} ${course.semesterName || ""}`));
    if (courses.length === 0) {
        return buildDataNotFoundResponse();
    }
    return {
        ...
        insights: [
            otherCourses[0] ? `Other enrolled courses include ${otherCourses.slice(0, 3).map((course)=>course.semesterName || course.courseTitle).join(", ")}.` : "No other enrolled courses were returned besides the selected course.",
            placementCourses[0] ? `Placement-related course found: ${placementCourses[0].courseTitle}.` : "No separate placement course was returned in the fetched course list."
        ].slice(0, 3)
    };
}
function buildSubjectPerformanceResponse(snapshot) {
    ...
    const labeledEntries = progressEntries.map((entry)=>buildSubjectPerformanceLabel(entry)).filter(Boolean);
    const tasks = progressEntries.slice(0, 3).map((entry)=>`Open ${entry.subjectName} in Newton to review the latest performance breakdown.`);
    return {
        summary: progressEntries.length === 1 ? `I found subject performance data for ${scope}.` : `I found subject-wise performance data for ${progressEntries.length} subjects in ${scope}.`,
        tasks,
...
function buildCalendarResponse(snapshot) {
    ...
    return {
        ...
        tasks: events.slice(0, 3).map((event)=>`Be ready for ${event.subjectName || event.title || event.type} on ${event.startsAt}.`),
        insights: [
            `The next calendar event is ${events[0].subjectName || events[0].title || events[0].type} on ${events[0].startsAt}.`,
            events[1] ? `The following event is ${events[1].subjectName || events[1].title || events[1].type} on ${events[1].startsAt}.` : null
        ].filter(Boolean)
    };
}
function buildTimelineResponse(snapshot) {
    ...
    const insights = [];
    if (recentLectures[0]) {
        tasks.push(recentLectures[0].hasRecording ? `Review the recording for ${truncateTitle(recentLectures[0].title || recentLectures[0].subjectName, 72)}.` : `Review the latest lecture notes for ${recentLectures[0].subjectName}.`);
        insights.push(`The most recent lecture was ${truncateTitle(recentLectures[0].title || recentLectures[0].subjectName, 72)} on ${recentLectures[0].startsAt}.`);
    }
...
function buildArenaResponse(snapshot) {
    ...
    return {
        summary: typeof arena.totalQuestions === "number" && arena.totalQuestions > 0 ? `Your Arena progress in ${scope} is ${arena.solvedCount}/${arena.totalQuestions} solved questions (${arena.solvedPercentage}%).` : `Arena data is available for ${scope}, but Newton returned no practice questions in this scope.`,
        tasks: tasks.slice(0, 3),
        insights: [
            typeof arena.todaySolvedCount === "number" ? `You have solved ${arena.todaySolvedCount} Arena questions today.` : null,
            typeof arena.solvedCount === "number" ? `Total solved Arena questions: ${arena.solvedCount}.` : null
        ].filter(Boolean)
    };
}
function buildQotdResponse(snapshot) {
    ...
    return {
        ...
        tasks: qotd.url ? [
            `Open today's QOTD in Newton: ${qotd.title}.`
        ] : [],
        insights: [
            typeof qotd.longestStreak === "number" ? `Your longest QOTD streak is ${qotd.longestStreak}.` : null,
            typeof qotd.streakLives === "number" ? `You currently have ${qotd.streakLives} streak lives left.` : null,
            typeof qotd.completedCount === "number" && typeof qotd.attemptedCount === "number" ? `${qotd.completedCount}/${qotd.attemptedCount} attempted submissions are marked completed for today's QOTD.` : null
        ].filter(Boolean)
    };
}
function buildExpertSessionsResponse(snapshot) {
    ...
    ].filter((item)=>/expert/i.test(`${item.title || ""} ${item.subjectName || ""} ${item.type || ""}`));
    if (candidates.length === 0) {
        return buildDataNotFoundResponse();
    }
    return {
        summary: `I found ${formatCountLabel(candidates.length, "expert session", "expert sessions")} in the fetched Newton data.`,
        tasks: candidates.slice(0, 3).map((item)=>`Open ${item.title || item.subjectName || "the expert session"} in Newton.`),
        insights: candidates.slice(0, 3).map((item)=>`${item.title || item.subjectName || "Expert session"}${item.startsAt ? ` starts on ${item.startsAt}` : ""}.`)
    };
}
function buildTodayResponse(snapshot) {
    ...
    const performance = snapshot.overview?.performance;
    if (!snapshot.context.subject && typeof performance?.lectures_attended === "number" && typeof performance?.total_lectures === "number" && performance.total_lectures > 0) {
...
function buildStoredNewtonPrompt(query, snapshotRecord) {
    return [
        "You are a JSON-only academic assistant.",
        "Academic data was fetched from Newton MCP by the backend and stored in Supabase before this reasoning step.",
        "You are reasoning ONLY over the stored database record provided below.",
        "Do NOT fetch tools, do NOT assume missing data, and do NOT use general knowledge.",
        "Use ONLY the stored academic data.",
        "Keep contests separate from assignments.",
        "Keep quizzes and assessments separate from both assignments and contests.",
        "If the stored data does not contain the requested information, return exactly {\"summary\":\"Data not found\",\"tasks\":[],\"insights\":[]}.",
        "If the stored data indicates a failure, return exactly {\"error\":\"MCP tool failed\"}.",
...
async function getNewtonSnapshot(query) {
    ...
    try {
        ...
        const courseSubjects = Array.isArray(context.course?.subjects) ? context.course.subjects : [];
        let overview = null;
        ...
        if (courseHash && (intent === "attendance" || intent === "performance" || intent === "rank" || intent === "contests" || intent === "today" || Boolean(context.subject))) {
        ...
        if (courseHash && (intent === "assignments" || intent === "today" || intent === "rank" || intent === "performance" || intent === "schedule" || intent === "contests")) {
        ...
        if (courseHash && (intent === "attendance" || intent === "today" || intent === "rank" || intent === "next_class" || intent === "performance" || intent === "schedule" || intent === "calendar" || intent === "timeline" || intent === "expert_sessions" || intent === "contests")) {
        ...
        if (courseHash && (context.subject || intent === "subject_performance")) {
            const subjectsToFetch = context.subject ? [
                context.subject
            ] : courseSubjects;
            for (const subject of subjectsToFetch){
                subjectProgresses.push(simplifySubjectProgress(subject, await client.callTool("get_subject_progress", {
                    course_hash: courseHash,
        ...
        if (courseHash && intent === "quizzes") {
            ...
            for (const subject of subjectsToFetch){
                assessments.push(simplifyAssessmentCollection(subject, await client.callTool("get_assessments", {
                    course_hash: courseHash,
                    subject_hash: subject.subject_hash
                })));
            }
        ...
        if (courseHash && (intent === "calendar" || intent === "timeline" || intent === "expert_sessions")) {
            calendar = simplifyCalendar(await client.callTool("get_calendar", {
                course_hash: courseHash,
                number_of_days: 7
            }));
        ...
        if (courseHash && (intent === "timeline" || intent === "expert_sessions")) {
            recentLectures = simplifyRecentLectures(await client.callTool("get_recent_lectures", {
                course_hash: courseHash,
                limit: 5
            }));
        ...
        if (courseHash && intent === "arena") {
            arena = simplifyArenaStats(await client.callTool("get_arena_stats", {
                course_hash: courseHash
            }));
            toolsUsed.push("get_arena_stats");
        }
        if (courseHash && intent === "qotd") {
            qotd = simplifyQuestionOfTheDay(await client.callTool("get_question_of_the_day", {
                course_hash: courseHash
            }));
            qotdHistory = simplifyQotdHistory(await client.callTool("get_qotd_history", {
                course_hash: courseHash,
...
async function POST(request) {
    ...
    try {
        ...
        const hasClaudeApiKey = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$claude$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["isClaudeConfigured"])();
        const hasSupabaseConfig = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["isSupabaseConfigured"])();
        if (!hasSupabaseConfig) {
            throw new Error("Supabase storage is required. Configure SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.");
        }
        if (!hasClaudeApiKey) {
            throw new Error("Claude reasoning is required. Configure ANTHROPIC_API_KEY or CLAUDE_API_KEY.");
        }
        ...
        const storedSnapshot = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["insertAcademicSnapshot"])({
            query,
        ...
        const persistedSnapshot = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getAcademicSnapshotById"])(storedSnapshot.id);
        const response = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$claude$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["askClaude"])((0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$newton$2d$mcp$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["buildStoredNewtonPrompt"])(query, persistedSnapshot));
        let parsedResponse;
        try {
        ...
        if (typeof parsedResponse.error === "string" && parsedResponse.error.trim()) {
            await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["updateAcademicSnapshotReasoning"])(persistedSnapshot.id, {
                reasoningResponse: {
                    error: parsedResponse.error.trim()
                },
                reasoningModel: (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$claude$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getConfiguredClaudeModel"])()
            });
            return Response.json({
                error: parsedResponse.error.trim()
            }, {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0grrev9._.js.map